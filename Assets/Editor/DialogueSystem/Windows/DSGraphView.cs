...
namespace DS.Windows
{
    ...
    public class DSGraphView : GraphView
    {
        ...
        public DSGroup CreateGroup(Vector2 position, string title = null)
        {
            var groupName = title == null ? GetFreeGroupName() : title;
            DSGroup group = new DSGroup(groupName, position);
        ...
        private string GetFreeGroupName()
        {
            int groupIndex = 0;
            while (true)
            {
                var possibleName = $"Dialogue group {groupIndex}";

                if (groups.ContainsKey(possibleName.ToLower()))
                {
                    groupIndex++;
                    continue;
                }
        ...
        public DSNode CreateNode(DSDialogueType dialogueType, Vector2 position, bool shouldDraw = true, string nodeName = null)
        {
            Type nodeType = Type.GetType($"DS.Elements.DS{dialogueType}Node");

            DSNode node = (DSNode) Activator.CreateInstance(nodeType);

            var freeNodeName = nodeName == null ? GetFreeNodeName() : nodeName;

            node.Initialize(freeNodeName, this, position);
        ...
        private string GetFreeNodeName()
        {
            int nodeIndex = 0;
            while (true)
            {
                var possibleName = $"Dialogue {nodeIndex}";

                if (ungroupedNodes.ContainsKey(possibleName.ToLower()) || groupedNodes.Values.Any(x => x.ContainsKey(possibleName.ToLower())))
                {
                    nodeIndex++;
                    continue;
                }
